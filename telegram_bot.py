...
emotion_keywords = {
    'грусть': [
        ...
        'расстроен', 'разочарован', 'обидно', 'жалко', 'тоска', 'подавленность',
        'подставил', 'предал', 'обманул'  # добавлено
    ],
    'тревога': [
        ...
        'жутко', 'тревожно', 'боязно', 'беспокоит', 'напрягает'
    ],
    'гнев': [
        ...
        'подрезали', 'обогнал', 'поворотник', 'хам', 'нарушил', 'достали',
        'подставил', 'напортачил', 'затупил', 'разрушить', 'ударить'  # добавлено
    ],
    'радость': [
        ...
        'зарплата', 'премия', 'успех', 'победа', 'подарок', 'приятно',
        'сдал', 'отчёт'  # добавлено
    ],
    'усталость': [
        ...
        'вымотался', 'утомительный', 'много работал'  # добавлено
    ]
}
...
responses = {
    'грусть': [
        ...
        "Иногда нужно просто выговориться, чтобы стало легче. Я слушаю.",
        "Понимаю, это неприятно. Хочешь обсудим?",
        "Ты справишься, я верю в тебя. Рассказывай.",
        "Обида и грусть часто рядом. Может, тебя кто-то подвёл?",
    ],
    'тревога': [
        ...
        "Попробуй описать своё состояние словами. Я пойму.",
        "Если хочешь, можем просто поболтать о чём-то приятном, чтобы отвлечься.",
        "Тревожно бывает, когда мы чего-то не контролируем. Что ты можешь сделать прямо сейчас?",
        "Даже если страшно, ты не один. Я здесь.",
    ],
    'гнев': [
        ...
        "Злость уходит, если её высказать. Давай, я слушаю.",
        "Иногда полезно просто поругаться вслух. Попробуй, станет легче.",
        "Когда нас подставляют, это бесит. Расскажи, как это случилось?",
        "Ты злишься, и это понятно. Но давай подумаем, как решить проблему.",
    ],
    'радость': [
        ...
        "Вот это новости! Очень приятно слышать.",
        "Здорово, когда всё хорошо. Что дальше планируешь?",
        "Радость — это круто! Давай обсудим подробности.",
        "Счастье любит тишину, но мне можно рассказать!",
        "Ты заслужил этот позитив. Молодец!",
    ],
    'усталость': [
        ...
        "Когда нет сил, важно себя поберечь. Что тебе помогает расслабиться?",
        "Усталость пройдёт, если дать себе время. Я подожду.",
        "После тяжёлого дня главное — выспаться. Позволь себе это.",
        "Ты молодец, что столько сделал. Теперь время отдыхать.",
        "Если хочешь, можем просто помолчать вместе. Я рядом.",
    ],
    'нейтрально': [
        "Рассказывай, я весь во внимании.",
        "Как сам? Что нового?",
        ...
        "Продолжай, я слушаю.",
        "Опиши подробнее, я хочу понять.",
        "Я открыт к любому разговору.",
        "Что у тебя нового? Поделишься?",
    ]
}
...
# Противительные союзы и слова сомнения
contrast_words = {'но', 'однако', 'хотя', 'зато', 'впрочем', 'а может', 'может быть'}
...
def extract_keywords(text):
    """Извлекает существительные (для возможного использования)"""
...
def analyze_emotion(text, prev_emotion=None):
    ...
    # Противительные союзы и слова сомнения
    contrast_present = any(phrase in text_lower for phrase in contrast_words) or any(word in contrast_words for word in words)
    ...
    for lemma in lemmas:
        for emotion, keywords_list in emotion_keywords.items():
            if lemma in keywords_list:
                # Если есть отрицание и слово из радости – это грусть
    ...
    # Если есть контраст, и предыдущая эмоция была сильной, возможно, стоит учесть?
    # Пока просто вернём доминирующую, но в будущем можно усложнить.
    return best_emotion

def get_response(emotion, user_id):
    possible = responses[emotion][:]  # копия списка
    user_data = user_context.get(user_id, {})
    last_responses = user_data.get('last_responses', [])

    if last_responses:
        # Фильтруем те, которые уже использовались недавно
        possible = [r for r in possible if r not in last_responses]
    if not possible:
        possible = responses[emotion]  # если все использованы, сбрасываем
...
async def cmd_help(message: types.Message):
    help_text = (
        ...
        "Я стараюсь запоминать контекст и не повторяться.\n\n"
        "📝 **Команды:**\n"
...
async def handle_message(message: types.Message):
    ...
    logger.info(f"User {user_id}: '{user_input[:50]}...' -> emotion: {emotion}")
